...
var _ColumnsForGroups = _interopRequireDefault(require("./columns/ColumnsForGroups"));
...
function (_Component) {
  ...
  function ReactCalendarTimeline(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "setLeftSidebarRef", function (el) {
      _this.sidebarComponent = el;
    });
    ...
    _defineProperty(_assertThisInitialized(_this), "resize", function () {
      ...
      var _this$container$getBo = _this.container.getBoundingClientRect(),
          containerWidth = _this$container$getBo.width; // let width = containerWidth - props.sidebarWidth - props.rightSidebarWidth


      var width = containerWidth - (_this.sidebarComponent ? _this.sidebarComponent.offsetWidth : 0) - props.rightSidebarWidth;
    ...
    _defineProperty(_assertThisInitialized(_this), "getTimeFromRowClickEvent", function (e) {
      ...
      var time; // For touch devices:

      if (offsetX === undefined) {
        var touchOffsetX = 0;

        if (typeof e.nativeEvent.touches !== 'undefined' && e.nativeEvent.touches.length > 0) {
          touchOffsetX = e.nativeEvent.touches[0].pageX;
        } else if (typeof e.nativeEvent.changedTouches !== 'undefined' && e.nativeEvent.changedTouches.length > 0) {
          touchOffsetX = e.nativeEvent.changedTouches[0].pageX;
        }

        var ratio = (0, _calendar.coordinateToTimeRatio)(_this.state.canvasTimeStart, _this.state.canvasTimeEnd, (0, _calendar.getCanvasWidth)(_this.state.width));
        var offset = (0, _domHelpers.getSumOffset)(_this.scrollComponent).offsetLeft;
        var scrolls = (0, _domHelpers.getSumScroll)(_this.scrollComponent);
        time = (touchOffsetX - offset + scrolls.scrollLeft) * ratio + _this.state.canvasTimeStart;
      } // For mouse events
      else {
          time = (0, _calendar.calculateTimeForXPosition)(canvasTimeStart, canvasTimeEnd, (0, _calendar.getCanvasWidth)(width), offsetX);
        }
    ...
    _defineProperty(_assertThisInitialized(_this), "dragItem", function (item, dragTime, oldGroup, dragGroupDelta) {
      var groups = _this.props.groups;
      var groupsKey = [];
      groupsKey = Object.keys(oldGroup);

      if (dragGroupDelta <= 0) {
        groupsKey.sort(function (a, b) {
          return oldGroup[a].index - oldGroup[b].index;
        });
      } else {
        groupsKey.sort(function (a, b) {
          return oldGroup[b].index - oldGroup[a].index;
        });
      }

      var newGroupOrder = [];
      var oldGroupOrder = groupsKey;
      var error = false;
      var groupsInsideItem = [];
      var elementPosition = 0;

      try {
        var sortOldGroups = Object.values(oldGroup).map(function (x) {
          return x.index;
        }).sort();

        for (var i = sortOldGroups[0]; i <= sortOldGroups.length + sortOldGroups[0]; i++) {
      ...
      groupsKey.map(function (gr) {
        ...
        while (typeof groups[oldGroup[gr].index + dragGroupDelta] !== 'undefined' && groups[oldGroup[gr].index + dragGroupDelta].root) {
          if (dragGroupDelta <= 0) {
            dragGroupDelta++;
          } else {
            dragGroupDelta--;
    ...
    _defineProperty(_assertThisInitialized(_this), "dropItem", function (item, dragTime, invisibleGroups) {
      _this.setState({
        draggingItem: null,
        dragTime: null,
        dragGroupTitle: null
      ...
      if (_this.props.onItemMove) {
        _this.props.onItemMove(item, dragTime, [].concat(_toConsumableArray(_this.state.newGroupOrder), _toConsumableArray(invisibleGroups)));
      }
    });
    ...
    _defineProperty(_assertThisInitialized(_this), "handleRowClickStart", function (e, rowIndex) {
      ...
      if (_this.props.onCanvasClickStart == null) return;
      ...
      _this.props.onCanvasClickStart(groupId, time, e);
    });

    _defineProperty(_assertThisInitialized(_this), "handleMouseMove", function (e, rowIndex) {
      if (_this.props.onMouseMove == null) return;
      ...
      if (_this.state.isMouseDown) {
        ...
        _this.props.onMouseMove(groupId, time, e);
      }
    ...
    _defineProperty(_assertThisInitialized(_this), "handleRowClickEnd", function (e, rowIndex) {
      if (_this.props.onCanvasClickEnd == null) return;
      ...
      _this.props.onCanvasClickEnd(groupId, time, e);
    });
  ...
  _createClass(ReactCalendarTimeline, [{
    ...
    value: function componentDidMount() {
      ...
      _window["default"].addListener(this); // window.addEventListener('scroll', this.handleMouseMove)


      this.lastTouchDistance = null;
      this.setState({
        sidebarWidth: !!this.sidebarComponent && this.sidebarComponent.offsetWidth
      });
    }
  }, {
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      if (!!this.sidebarComponent && this.state.sidebarWidth !== this.sidebarComponent.offsetWidth) {
        this.resize();
        this.setState({
          sidebarWidth: this.sidebarComponent.offsetWidth
        });
      }
    }
  }, {
    key: "columnsForGroups",
    value: function columnsForGroups(canvasTimeStart, canvasTimeEnd, canvasWidth, minUnit, timeSteps, height, groups, groupHeights) {
      return _react["default"].createElement(_ColumnsForGroups["default"], {
        canvasTimeStart: canvasTimeStart,
        canvasTimeEnd: canvasTimeEnd,
    ...
    value: function sidebar(height, groupHeights) {
      ...
      return sidebarWidth && _react["default"].createElement(_Sidebar["default"], {
        ...
        groupHeights: groupHeights,
        height: height,
        sidebarRef: function sidebarRef(el) {
          return _this3.setLeftSidebarRef(el);
        }
      });
    }
  }, {
    ...
    value: function render() {
      ...
      return _react["default"].createElement(_TimelineStateContext.TimelineStateProvider, {
        ...
        className: "rct-outer"
      }, (this.sidebarComponent ? this.sidebarComponent.offsetWidth : this.props.sidebarWidth) > 0 ? this.sidebar(height, groupHeights) : null, _react["default"].createElement(_ScrollElement["default"], {
        scrollRef: this.getScrollElementRef,
        width: width,
        height: height
      }, _react["default"].createElement(_MarkerCanvas["default"], null, this.columnsForGroups(canvasTimeStart, canvasTimeEnd, canvasWidth, minUnit, timeSteps, height, groups, groupHeights), this.rows(canvasWidth, groupHeights, groups), this.items(canvasTimeStart, zoom, canvasTimeEnd, canvasWidth, minUnit, dimensionItems, groupHeights, groupTops), this.childrenWithProps(canvasTimeStart, canvasTimeEnd, canvasWidth, dimensionItems, groupHeights, groupTops, height, visibleTimeStart, visibleTimeEnd, minUnit, timeSteps))), rightSidebarWidth > 0 ? this.rightSidebar(height, groupHeights) : null)))));
    }
  }], [{
...
_defineProperty(ReactCalendarTimeline, "propTypes", {
  ...
  onCanvasClickStart: _propTypes["default"].func,
  onCanvasClickEnd: _propTypes["default"].func,
  ...
  verticalLineClassNamesForGroups: _propTypes["default"].func,
