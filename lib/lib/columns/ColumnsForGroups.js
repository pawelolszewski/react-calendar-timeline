...
var ColumnsForGroups =
/*#__PURE__*/
function (_Component) {
  _inherits(ColumnsForGroups, _Component);

  function ColumnsForGroups() {
    _classCallCheck(this, ColumnsForGroups);

    return _possibleConstructorReturn(this, _getPrototypeOf(ColumnsForGroups).apply(this, arguments));
  }

  _createClass(ColumnsForGroups, [{
    key: "shouldComponentUpdate",
    value: function shouldComponentUpdate(nextProps) {
      return !(nextProps.canvasTimeStart === this.props.canvasTimeStart && nextProps.canvasTimeEnd === this.props.canvasTimeEnd && nextProps.canvasWidth === this.props.canvasWidth && nextProps.lineCount === this.props.lineCount && nextProps.minUnit === this.props.minUnit && nextProps.timeSteps === this.props.timeSteps && nextProps.height === this.props.height && nextProps.verticalLineClassNamesForGroups === this.props.verticalLineClassNamesForGroups);
    }
  }, {
    key: "render",
    value: function render() {
      var _this$props = this.props,
          ...
          verticalLineClassNamesForGroups = _this$props.verticalLineClassNamesForGroups,
          getLeftOffsetFromDate = _this$props.getLeftOffsetFromDate,
          groups = _this$props.groups,
          groupHeights = _this$props.groupHeights;
      var gTop = 0;
      var lines = [];
      groups && groups.reduce(function (pV, cV, cI) {
        if (cV.root) {
          pV.push({
            id: cV.id,
            top: gTop,
        ...
        (0, _calendar.iterateTimes)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
          ...
          if (verticalLineClassNamesForGroups) {
            classNamesForGroup = verticalLineClassNamesForGroups(time.unix() * 1000, // turn into ms, which is what
            // verticalLineClassNamesForGroups expects
            nextTime.unix() * 1000 - 1, group.group);
          } // TODO: rename or remove class that has reference to vertical-line


          var classNames = 'rct-vl' + (firstOfType ? ' rct-vl-first' : '') + (minUnit === 'day' || minUnit === 'hour' || minUnit === 'minute' ? " rct-day-".concat(time.day(), " ") : '') + classNamesForGroup.join(' ');
          ...
          lines.push(_react["default"].createElement("div", {
            key: "line-group-".concat(group.id, "-").concat(time.valueOf()),
            className: classNames,
            style: {
              pointerEvents: 'none',
              top: "".concat(group.top, "px"),
              left: "".concat(left, "px"),
              width: "".concat(right - left, "px"),
              height: "".concat(group.height, "px")
            }
          }));
        });
      }); // debugger
      ...
      return _react["default"].createElement("div", {
        className: "rct-vertical-lines-for-groups"
      }, lines);
    }
  }]);

  return ColumnsForGroups;
}(_react.Component);

_defineProperty(ColumnsForGroups, "propTypes", _objectSpread({}, passThroughPropTypes, {
...
var ColumnsWrapper = function ColumnsWrapper(_ref) {
  ...
  return _react["default"].createElement(_TimelineStateContext.TimelineStateConsumer, null, function (_ref2) {
    ...
    return _react["default"].createElement(ColumnsForGroups, _extends({
