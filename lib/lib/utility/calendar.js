...
function getGroupedItems(items, groupOrders) {
  ...
  var _loop = function _loop(_i) {
    if (items[_i].dimensions.order !== undefined) {
      items[_i].dimensions.order.forEach(function (el) {
        var groupItem = groupedItems[el.index];
...
function getCanvasWidth(width) {
  var buffer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  return width * buffer;
}
/**
...
function getItemDimensions(_ref3) {
  ...
  if (dimension) {
    var group = (0, _generic._get)(item, keys.itemGroupKey);
    var order = [];
...
function getCanvasBoundariesFromVisibleTime(visibleTimeStart, visibleTimeEnd) {
  var canvasTimeStart = visibleTimeStart;
  var canvasTimeEnd = visibleTimeEnd;
